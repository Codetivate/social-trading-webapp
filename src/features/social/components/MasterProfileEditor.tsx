...
export function MasterProfileEditor({ onClose, role, profile, setProfile, userImage, openConfirm }: MasterProfileEditorProps) {
    ...
    const previewData: Master = {
        ...
        desc,
        tags,
        avatar: avatar || userImage || `https://api.dicebear.com/7.x/avataaars/svg?seed=${name}`,
        pnlText: "+$12,450",
        followers: 5420,
        risk: 6,
        winRate: 88,
    ...
    return (
        <div className="space-y-6 animate-in fade-in">
            ...
            <div className="grid grid-cols-2 gap-3 pt-4 border-t border-gray-800"><button onClick={() => setPreviewMode(true)} disabled={isSaving} className="bg-gray-800 hover:bg-gray-700 text-white text-xs font-bold py-3 rounded-xl flex items-center justify-center gap-2"><Edit3 size={16} /> Preview</button><button onClick={handleSave} disabled={isSaving} className="bg-purple-600 hover:bg-purple-500 text-white text-xs font-bold py-3 rounded-xl flex items-center justify-center gap-2 shadow-lg shadow-purple-900/20">{isSaving ? "Saving..." : <><Save size={16} /> Save Changes</>}</button></div>
        </div>
    );
}
