        ...
        try:
            ...
            for m_ticket, m_info in master_positions.items():
                ...
                if is_missing:
                    ...
                    # 🛡️ SLIPPAGE PROTECTION (0.05%)
                    try:
                        tick = mt5.symbol_info_tick(m_info['symbol'])
                        if tick:
                            open_price = float(m_info['price'])
                            is_buy = (m_info['type'] == mt5.ORDER_TYPE_BUY)
                            
                            if is_buy:
                                # Buying: If Ask is too high (> +0.05%), skip.
                                current_price = tick.ask
                                deviation = (current_price - open_price) / open_price
                                if deviation > 0.0005: # 0.05%
                                     print(f"   [SKIP] Slippage High on BUY {m_ticket}. Dev: {deviation*100:.4f}% > 0.05%")
                                     continue
                            else:
                                # Selling: If Bid is too low (< -0.05%), skip.
                                current_price = tick.bid
                                deviation = (open_price - current_price) / open_price # Postive deviation means drop
                                if deviation > 0.0005: 
                                     print(f"   [SKIP] Slippage High on SELL {m_ticket}. Dev: {deviation*100:.4f}% > 0.05%")
                                     continue
                    except Exception as e:
                        print(f"   [WARN] Slippage Check Failed: {e}")
                    ...
                    if EXECUTION_MODE in ['BATCH', 'TURBO']:
                        ...
                        # We assume if SINGLE mode, we are the follower.
                        f_id = MY_FOLLOWER_ID
                        execute_trade(catchup_signal, api_url or "http://localhost:3000", follower_id=f_id)
...
def run_executor():
    ...
    print(f"[INFO] Hydra Execution Engine v1.1")
