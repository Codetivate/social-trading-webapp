...
def initialize_mt5():
    ...
    if not mt5.initialize(**init_params):
        ...
        if EXECUTION_MODE == 'SINGLE':
            ...
            if creds == "FATAL_404":
                print("[STOP] CRITICAL: Broker Account Not Found (404). Aborting.")
                mt5.shutdown()
                quit()
...
def initialize_mt5():
    ...
    if not mt5.initialize(**init_params):
        ...
        if EXECUTION_MODE == 'SINGLE':
            creds = fetch_credentials()
            if creds == "FATAL_404":
                print("[FATAL] 🛑 Broker Account Not Found (404). Exiting.")
                if not DRY_RUN: quit()
            elif creds:
                 if not login_mt5(creds):
                     print("[WARN] Warning: Could not login to Follower Account.")
    ...
    for sub_master_id in subs: # sub_master_id is just the master ID string
        ...
        try:
            ...
            if MY_FOLLOWER_ID:
                 ...
                 if creds == "FATAL_404":
                     print(f"   [FATAL] 🛑 Account {MY_FOLLOWER_ID} Not Found. Unrecoverable. Exiting.")
                     import os
                     os._exit(1) # Force Exit to stop Zombie Loop
                     ...
                     if current_info.login != target_login:
