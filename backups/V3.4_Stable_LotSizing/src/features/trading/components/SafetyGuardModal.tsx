...
export function SafetyGuardModal({
    ...
    initialInvert = false,
    initialCopyMode = "FIXED", // 🛠️ DEBUG/FIX: Default to Fixed Ratio
    onClose,
    onConfirm,
    maxAlloc,
    showWelcomeOption
    ...
    const [hasAgreed, setHasAgreed] = useState(false); // Legal Consent State
    ...
    return (
        <div className="fixed inset-0 z-60 bg-black/80 backdrop-blur-sm flex items-end sm:items-center justify-center p-0 sm:p-4 animate-in fade-in">
            <div className="bg-gray-950 w-full sm:max-w-md sm:rounded-3xl border-t sm:border border-gray-800 shadow-2xl flex flex-col max-h-[90vh] sm:max-h-[85vh] h-full sm:h-auto relative overflow-hidden" onClick={(e) => e.stopPropagation()}>
                ...
                {/* STICKY FOOTER */}
                <div className="flex-none p-4 bg-gray-950 border-t border-gray-900 z-20 space-y-4">

                    {/* ⚠️ LEGAL CONSENT CHECKBOX */}
                    <div className="bg-gray-900/50 p-3 rounded-xl border border-gray-800 animate-in fade-in slide-in-from-bottom-2">
                        <label className="flex items-start gap-3 cursor-pointer group">
                            <div className="relative flex items-center">
                                <input
                                    ...
                                    className="peer h-5 w-5 bg-gray-950 border-2 border-gray-600 rounded-md checked:bg-green-500 checked:border-green-500 transition-all cursor-pointer appearance-none shrink-0"
                                />
                                <CheckCircle2 size={12} className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 text-black opacity-0 peer-checked:opacity-100 pointer-events-none" />
                            </div>
                            <div className="space-y-1">
                                <p className="text-xs text-gray-300 leading-snug font-sans group-hover:text-white transition-colors">
                                    I verify that SignalTrade is strictly <strong>Technical Cloud Infrastructure</strong>, not a financial advisor. I accept all trading risks.
                                </p>
                                <div className="flex gap-2 text-[10px] text-gray-500 font-mono">
                                    <a href="/legal/risk-disclaimer" target="_blank" className="hover:text-blue-400 underline decoration-gray-700">Risk Disclosure</a>
                                    <span>•</span>
                                    <a href="/legal/terms-of-service" target="_blank" className="hover:text-blue-400 underline decoration-gray-700">Terms & Conditions</a>
                                </div>
                            </div>
                        </label>
                    </div>
                    ...
                    <button
                        ...
                        disabled={!hasAgreed}
                        className="w-full bg-white active:bg-gray-200 disabled:opacity-50 disabled:cursor-not-allowed text-black font-bold py-3 rounded-xl text-sm uppercase tracking-wide shadow-xl flex items-center justify-center gap-2 hover:scale-[1.02] transition-transform"
                    >
                        <CheckCircle2 size={16} className={hasAgreed ? "text-green-600" : "text-gray-400"} /> Confirm & Start
                    </button>
                </div>
            </div>
        </div>
