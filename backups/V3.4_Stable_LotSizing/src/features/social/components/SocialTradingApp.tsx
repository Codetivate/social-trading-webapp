                    ...
                    <div className="space-y-4 pb-24">
                        ...
                        {activeSessions.length === 0 ? (<div className="text-center py-10 text-gray-500 space-y-2 border border-dashed border-gray-800 rounded-2xl"><Briefcase className="mx-auto opacity-50" size={40} /><p className="text-xs">No active copies.</p></div>) : (
                            activeSessions.map((session) => {
                                ...
                                // If user has only 1 active session, we assume ALL open positions belong to this session.
                                // We trust realTimePositions if it exists (even if empty => 0 PnL).
                                // This ensures that if all trades are closed, we show 0 instead of stale cached data.
                                let rtPnL = undefined;
                                if (activeSessions.length === 1 && realTimePositions) {
                                    rtPnL = realTimePositions.reduce((sum: number, p: any) => sum + (p.profit || 0), 0);
                                }
                    ...
                    <div className="flex flex-col md:flex-row items-center gap-6">
                        ...
                        <div className="flex items-center gap-8 md:gap-12">
                            ...
                            <div className="text-center">
                                ...
                                <p className="text-xl font-mono font-bold text-gray-300">2026</p>
                            </div>
                        </div>
                    </div>
