    ...
    try:
        ...
        if EXECUTION_MODE == 'BATCH':
            ...
            for row in rows:
                ...
                batch_subs[mid_str].append({
                    ...
                    'copy_mode': cfg.get('copyMode', 'FIXED') if isinstance(cfg, dict) else 'FIXED'
                })
        ...
        # print(f"[DEBUG] Fetching ALL subscriptions for Dynamic HFT Swarm...")
            ...
            for row in rows:
                ...
                # 🛠️ PARSE CONFIG (Handle String types)
                # Must do this BEFORE creating the item dict
                raw_cfg = cfg
                if isinstance(cfg, str):
                    try: 
                        ...
                        print(f"[ERROR] JSON Parse Failed for: {raw_cfg}")
                        cfg = {}
                
                mode = cfg.get('copyMode', 'FIXED') if isinstance(cfg, dict) else 'FIXED'
                
                # 🩹 BRUTE FORCE FALLBACK
                if mode == 'FIXED' and isinstance(raw_cfg, str) and 'EQUITY' in raw_cfg.upper():
                     print(f"[WARN] JSON passed but mode missing? Found EQUITY in raw string. Forcing EQUITY.")
                     mode = 'EQUITY'

                # Debug Check (Unconditionally Report for now to fix user issue)
                # print(f"[DEBUG-CFG] Config: {raw_cfg} (Type: {type(raw_cfg)}) -> Mode: {mode}")
            ...
            query = """
                SELECT "masterId", "timeConfig", "expiry", "riskFactor", "invertCopy", "createdAt", "allocation" FROM "CopySession"
                WHERE "followerId" = %s 
                  AND "isActive" = true
            ...
            # row: 0=mid, 1=cfg, 2=expiry, 3=riskFactor, 4=invertCopy, 5=createdAt, 6=allocation
            return {
                str(row[0]): {
                    'config': row[1], 
                    'expiry': row[2], 
                    ...
                    'copy_mode': row[1].get('copyMode', 'FIXED') if isinstance(row[1], dict) else 'FIXED',
                    'allocation': float(row[6] or 0.0) # 🆕 Added Allocation
                } for row in rows
            }
            ...
            for m_ticket_str, m_info in master_positions.items():
                ...
                if not local_p:
                         ...
                         catchup_signal = {
                             ...
                             "master_equity": master_equity_snapshot # 🛠️ CRITICAL: Pass Master Eq for Ratio Calc
                         }
