...
def calculate_safe_lot(master_lot_size, equity, leverage, risk_factor_percent, symbol="EURUSD"):
    ...
    Args:
        master_lot_size (float): The original lot size from the Master.
        equity (float): Follower's current Account Equity.
        leverage (int): Follower's Account Leverage (e.g., 500).
        risk_factor_percent (float): User setting (e.g. 100 = 1:1, 50 = 0.5x, 200 = 2.0x).
        symbol (str): Symbol to check specific contract specs (Min/Max/Step).
    
    Returns:
        float: The final safe lot size to trade.
    """
    # 1. Basic Pro-Rata Logic
    # If risk_factor is 100, we mimic master lot? 
    # NO. "Pro-Rata" usually means relative to ratio or just a multiplier.
    # The user said "Zulutrade-style". 
    # If Master trades 0.1 and User sets 50%, User trades 0.05.
    # We treat risk_factor_percent as a simple multiplier on Volume for now (MVP).
    # If user wants "Ratio" (myEquity / masterEquity), that's harder without knowing Master Equity.
    # Assuming "Fixed Ratio" multiplier for now based on UI "100%", "50%".
    ...
    if not sym_info:
        print(f"   [WARN] Symbol Info not found for {symbol}. Using defaults.")
    ...
    # Handle massive contract sizes (Index/Crypto) if needed
    if contract_size < 1: contract_size = 100000 # Fallback standard lot
    
    if leverage < 1: leverage = 500 # Default fallback
    
    needed_margin = (final_lot * contract_size * price) / leverage
    
    # 🛡️ SAFETY: Do we have enough Free Margin?
    ...
    if account:
        ...
        if free_margin < needed_margin:
            ...
            # 🛑 USER PREFERENCE: Force MIN LOT instead of Skip?
            # User said: "margin safety adjustment... force minimum lot size"
            if free_margin > (min_lot * contract_size * price) / leverage:
                print(f"   [RISK] 📉 Downgrading to MIN LOT ({min_lot}) to survive.")
                return min_lot
            else:
                 print(f"   [RISK] ❌ CRITICAL: Cannot afford even Min Lot. Skipping.")
                 return 0.0
...
def process_execution_report(report, signal, login_map):
    ...
    for res in report:
        ...
        try:
             ...
             if success:
                 deal_data = res.get('deal_data') or {}
                 history_item = {
                     "ticket": str(local_ticket),
                     "order": str(local_ticket), # For follower, order~ticket
                     "time": int(time.time()), # Timestamp
                     "type": 0 if signal.get('type', 'BUY') == 'BUY' else 1, # 0=Buy, 1=Sell
                     "entry": 1 if signal.get('action') == 'CLOSE' else 0, # 0=In, 1=Out
                     "symbol": signal.get('symbol'),
                     "volume": float(deal_data.get('volume', payload['volume'])),
                     "price": float(deal_data.get('price', payload['price'])),
                 ...
                 # Send only on CLOSE or if you want to track Opens too. History usually tracks Closes (profit).
                 # To match Broadcaster, we track Deals.
                 requests.post(f"{api_url}/api/webhook/history-batch", json={
                     "history": [history_item],
                     "masterId": "IMPORTED" # Special flag or empty. For Follower, we might use followerId in different endpoint?
                     # Wait, /history-batch endpoint expects `masterId` but also processes array. 
                     # The backend likely links it to the USER who sent the request.
                     # But here we act as the System.
                     # Actually, `route.ts` uses `x-user-id` header to determine owner.
                 }, headers={"x-bridge-secret": "AlphaBravoCharlieDeltaEchoFoxtro", "x-user-id": f_id}, timeout=2)
...
# 💀 DEAD CODE REMOVED (Duplicate reconcile_initial_state)
# See line ~3200 for Active Definition

# 💀 DEAD CODE (Duplicate Definition) - See Line ~3700 for Active Code
# 💀 DEAD CODE REMOVED (Conflicting Duplicate)
