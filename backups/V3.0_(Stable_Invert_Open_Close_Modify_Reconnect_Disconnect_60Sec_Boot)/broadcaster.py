        ...
        try:
            ...
            if master_creds:
                 ...
                 if not start_info or str(start_info.login) != str(target_login):
                     ...
                     if r_client:
                         ...
                         # 🛡️ YIELD LOGIC: If Locked by SOMEONE ELSE
                         # If I hold the lock, I must enforce my state.
                         # If someone else holds the lock, I yield.
                         if current_lock and current_lock != USER_ID:
                               # print(f"[INFO] Account switched by HFT ({current_lock}). Yielding...")
                               time.sleep(2.0) # WAIT for Worker to finish
                               continue # Yield loop
                     
                     # If we get here, either WE hold the lock, or NO ONE holds the lock.
                     # But account is wrong. We MUST switch it.
            ...
            if closed_tickets:
                ...
                for ticket in closed_tickets:
                    ...
                    if history:
                        for deal in history:
                            # 🛡️ ROBUST MATCH: Support OUT (Standard), OUT_BY (Close By), INOUT (Reversal)
                    ...
                    try:
                        history_item = {
                            ...
                            "price": float(deal_info.price) if deal_info else c_price,
